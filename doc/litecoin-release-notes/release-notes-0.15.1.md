Suscoin Core version *0.15.1* is now available from:

  <https://download.suscoin.org/suscoin-0.15.1/>
...
A number of changes to the way Suscoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 crew per kB (0.00001 SUS). This is to prepare for dropping the minimum transaction fee to 0.1 crew per kB (0.0001 SUS) in the next release.
